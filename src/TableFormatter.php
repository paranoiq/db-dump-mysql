    ...
    private function calculateColumnWidths(array $rows, int $tableWidth, $columnsCount, array $formats): array
    {
        ...
        foreach ($dateTimeColumnWidths as $i => $width) {
            if (($formats[$i] === 'datetime' || $formats[$i] === 'date') && ($width === 8 || $width === 9)) {
                $columnWidths[$i] = 10;
                $columnWidths[array_search(max($nonDateTimeColumnWidths), $nonDateTimeColumnWidths)] -= (10 - $width);
            } elseif (($formats[$i] === 'datetime' || $formats[$i] === 'date') && $width === 6) {
                $columnWidths[$i] = 7;
                $columnWidths[array_search(max($nonDateTimeColumnWidths), $nonDateTimeColumnWidths)]--;
            } elseif ($formats[$i] === 'time' && ($width === 6 || $width === 7)) {
                $columnWidths[$i] = 8;
                $columnWidths[array_search(max($nonDateTimeColumnWidths), $nonDateTimeColumnWidths)] -= (8 - $width);
            }
        }
        ...
        foreach ($columnWidths as $i => $width) {
            if ($width === 0) {
                $columnWidths[$i] = 1;
                $columnWidths[array_search(max($columnWidths), $columnWidths)]--;
            }
        }
